 ...
 * @param {Uint8Array} tx_hash
 * @returns {Uint8Array}
 */
export function secure_sign(tx_hash) {
    const ptr0 = passArray8ToWasm0(tx_hash, wasm.__wbindgen_malloc);
    const len0 = WASM_VECTOR_LEN;
    const ret = wasm.secure_sign(ptr0, len0);
    if (ret[3]) {
        throw takeFromExternrefTable0(ret[2]);
    }
 ...
 * @returns {Uint8Array}
 */
export function sign_with_password(password, salt, tx_hash) {
    const ptr0 = passArray8ToWasm0(password, wasm.__wbindgen_malloc);
    const len0 = WASM_VECTOR_LEN;
    const ptr1 = passStringToWasm0(salt, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
    const len1 = WASM_VECTOR_LEN;
    const ptr2 = passArray8ToWasm0(tx_hash, wasm.__wbindgen_malloc);
    const len2 = WASM_VECTOR_LEN;
    const ret = wasm.sign_with_password(ptr0, len0, ptr1, len1, ptr2, len2);
    if (ret[3]) {
        throw takeFromExternrefTable0(ret[2]);
    }
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('wasm_vault_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
